  ...
  if (i<topSites.length){
    ...
    chrome.kill().then(()=>{
      lock.writeLock(function (release) {
        console.log("writing to file",path.join(dirPath, 'lighthouseTTB' + dir + fileNumber + ".json"))
        fs.appendFile(path.join(dirPath, 'lighthouseTTB' + dir + fileNumber + ".json"), JSON.stringify(dict,null,4),(err)=>{
        if(err) 
