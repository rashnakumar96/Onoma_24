...
class WebPerformanceTests:
	...
	def runLighthouse(self,approach,_resources,c):
		...
		# _ = stream.read()
		print ("Length of chunk: ",len(_resources),str(c))
		call(["node", join(project_path, "runLighthouse.js"), self.countryPath, approach, str(c)] + _resources)
...
if __name__ == "__main__":
	...
	if not os.path.exists(join("measurements", country)):
		os.mkdir(join("measurements", country))

	publicDNSServers=[]
	allpublicDNSServers = json.load(open(join(project_path, "country_public_dns.json")))
	...
	with open(join(project_path, "measurements", country, "publicDNSServers.json"),'w') as fp:
