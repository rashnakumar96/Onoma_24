...
def plotK_ValuesPerSharding(Injected,z,filename,jaccard_value,user):
	alphas=[1,0.75,0.5,0.25]
	...
	avgKdict={}
	# dataFile=json.load(open("InjectedReqsTemp/Injectedsharding_value"+str(z)+".json"))
	# InjectedReqsTemp/Injectedsharding_valueweek1_7_25_8_3_users_800Result.json
	# resFile=json.load(open("InjectedReqsTemp/Injectedsharding_value"+filename+'_'+str(z)+'_'+str(k)+"_users_"+str(user)+str(jaccard_value)+"Result.json"))
	for i in insertion_values:
		avgKdict["insertion_value"+str(z)]={}
		if Injected:
			resFile=json.load(open("InjectedReqsTemp/Injectedsharding_value"+filename+'_'+str(z)+'_'+str(i)+"_users_"+str(user)+str(jaccard_value)+"Result.json"))
		else:
			resFile=json.load(open("sharding_value"+filename+str(z)+"_users_"+str(800)+"Result.json"))
		r=0
		c=0
		avgDict={}
		for run in resFile:
	...
	if not os.path.exists("InjectedReqsTemp/graphs"):
		os.mkdir("InjectedReqsTemp/graphs")
	if Injected:
		plt.savefig("InjectedReqsTemp/graphs/InjectedK_valuesCDF_"+filename+"_sharding_"+str(z)+"_users_"+str(800))
	else:
		plt.savefig("graphs/K_valuesCDF"+filename+"_users_"+str(800))
	plt.clf()
	print (avgKdict)
...
if __name__ == "__main__":
	...
	for i in insertion_values:
		...
		plotK_ValuesPerInsertion(True,i,filename,0.3,{},user)
	...
	for z in sharding_values:
		plotK_ValuesPerSharding(True,z,filename,0.3,user)
